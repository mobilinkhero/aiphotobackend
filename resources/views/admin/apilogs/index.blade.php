<x-app-layout>
    ...
    <!-- Pass data safely via Script to avoid breaking HTML attributes with large JSON -->
    <script>
        window.apiLogsData = @json($logs->values());
    </script>

    <div x-data="{ 
        open: false, 
        selected: { path: '', request: '', response: '', status: '', method: '' },
        logs: window.apiLogsData || [],
        ...
        <div class="card overflow-hidden shadow-sm">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                ...
                <span class="text-[10px] text-blue-600 font-bold bg-blue-50 px-2 py-1 rounded border border-blue-100">LIVE FEED</span>
            </div>
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($logs as $index => $log)
                        <tr class="hover:bg-blue-50/50 transition-all cursor-pointer group" @click="showLog({{ $index }})">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <button type="button" @click.stop="showLog({{ $index }})" 
                                    class="bg-blue-600 text-white text-[10px] font-bold px-4 py-2 rounded-lg shadow hover:bg-blue-700 active:scale-95 transition-all uppercase">
                                    Peek Log
                                </button>
                            </td>
                        </tr>
            ...
            <div x-show="open" 
                ...
                <div class="bg-white rounded-3xl shadow-[0_35px_100px_-15px_rgba(0,0,0,0.8)] max-w-5xl w-full max-h-[90vh] flex flex-col overflow-hidden border border-gray-100" 
                    ...
                    <div class="px-8 py-6 border-b flex justify-between items-center bg-gray-50/50 backdrop-blur-sm">
                        <div>
                            ...
                            <div class="flex items-center gap-3 mt-2">
                                <span class="text-[10px] font-mono px-3 py-1 rounded-md bg-white border border-gray-200 shadow-sm text-gray-500" x-text="'HTTP ' + selected.status"></span>
                                <span class="text-[10px] font-black px-3 py-1 rounded-md border" 
                                      :class="{
                        ...
                        <button @click="open = false" class="bg-white hover:bg-red-50 hover:text-red-500 text-gray-400 rounded-full w-12 h-12 flex items-center justify-center text-3xl font-light shadow-sm transition-all">&times;</button>
                    </div>

                    <div class="p-8 overflow-y-auto space-y-10 bg-white">
                        <div>
                            <h3 class="text-[10px] font-black text-blue-600 mb-4 uppercase tracking-[0.4em] flex items-center gap-3">
                                <span class="w-2 h-2 rounded-full bg-blue-600 shadow-lg shadow-blue-500/50"></span>
                                Mobile Application (Payload Sent)
                            </h3>
                        ...
                        <div x-show="getAiLogs()">
                            <h3 class="text-[10px] font-black text-orange-600 mb-4 uppercase tracking-[0.4em] flex items-center gap-3">
                                <span class="w-2 h-2 rounded-full bg-orange-600 shadow-lg shadow-orange-500/50"></span>
                                Internal AI Workflow (Replicate)
                            </h3>
                        ...
                        <div>
                            <h3 class="text-[10px] font-black text-green-600 mb-4 uppercase tracking-[0.4em] flex items-center gap-3">
                                <span class="w-2 h-2 rounded-full bg-green-600 shadow-lg shadow-green-500/50"></span>
                                Server Response (Sent to App)
                            </h3>
                    ...
                    <div class="px-8 py-6 border-t bg-gray-50 flex justify-end">
                        <button @click="open = false" class="bg-gray-900 text-white text-xs font-black px-12 py-4 rounded-2xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-widest">Close Dashboard</button>
                    </div>
                </div>
            </div>
        </template>
